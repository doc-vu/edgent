...
brokers=['node2']
publisher_test_machines=['node%d'%(i) for i in range(3,41,1)]
#publisher_test_machines+=['node28','node29','node30','node31']
#publisher_test_machines=['node3']
subscriber_test_machines=['node41']
...
#publisher_test_machines.remove('node23')

#subscriber_test_machines.remove('node23')
publisher_test_machines.remove('node23')

#subscriber_test_machines.remove('node27')
publisher_test_machines.remove('node27')

#subscriber_test_machines.remove('node34')
publisher_test_machines.remove('node34')

#subscriber_test_machines.remove('node39')
publisher_test_machines.remove('node39')

#pub_per_hw_type={'hw1':20,'hw2':40}
#sub_per_hw_type={'hw1':50,'hw2':100,'hw3':150,'hw4':200,'hw5':200}
sub_per_hw_type={'hw1':10,'hw2':20,'hw3':35,'hw4':45,'hw5':45}
#sub_per_hw_type={'hw1':2,'hw2':2,'hw3':2,'hw4':2,'hw5':2}
pub_per_hw_type={'hw1':10,'hw2':20,'hw3':35,'hw4':45,'hw5':45}
...
if __name__ == "__main__":
  ...
  for region_car in args.region_cars:
    ...
    #  (region,region,sample_count,region,cars,sample_count,cars,cars+1,4000))
    #config.append('t%d,%d,t%d:%d:1:%d:4000,t%d:1:%d,%d,1,%d,%d'%\
    #  (region,cars,region,cars,sample_count,region,cars*sample_count,cars,cars+1,4000))
    ...
    #  (region,cars,region,cars,region,cars,cars+1,4000))
    config.append('t%d,%d,t%d:%d:1:%d:4000,t%d:1:%d,%d,1,%d,%d'%\
      (region,cars,region,cars,sample_count,region,cars*sample_count,cars,cars+1,4000))
    #config.append('t1,1,t1:1:1:%d:4000,t1:%d:%d,1,%d,%d,100'%\
    #  (sample_count,cars,sample_count,cars,cars+1))
  ...
  if check(config):
    ...
    map(lambda x: client_and_broker_machines.append(x),foreground_background_sub['background'].keys()) 
    util.collect_logs(run_id,args.log_dir,','.join(client_and_broker_machines))

    ##copy model features 
    #print("\n\nCopying model features")
    #features=model_features(config)
    #with open('%s/%s/features'%(args.log_dir,run_id),'w') as f:
    ...
    #with open('%s/%s/config'%(args.log_dir,run_id),'w') as f:
    ...
    subprocess.check_call(['python','src/plots/summarize/summarize.py',\
      '-log_dir',args.log_dir,'-sub_dirs',str(run_id)])
