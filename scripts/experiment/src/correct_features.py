...
colocated_topics=[8,9,10,11,12,13,14,15]

for count in colocated_topics:
  ...
  for iteration in sorted_dirs:
    config=load_configuration('%s/colocated_with_proc_%d/%d/config'%\
      (log_dir,count,iteration))
    ...
    with open('%s/colocated_with_proc_%d/%d/features'%(log_dir,count,iteration),'w') as f:
      f.write('iter,background_topics,foreground_processing_interval,foreground_rate,background_rate_x_processing_interval,\
