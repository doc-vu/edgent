...
import restart3,rate_processing_interval_combinations3
...
def run(run_id,topics,models_dir,log_dir,req=None):
  ...
  util.start_eb(','.join(rate_processing_interval_combinations3.brokers),zk_connector)

  #run test
  print('Starting test') 
  config=rate_processing_interval_combinations3.\
        create_configuration(req)
  restart3.Hawk(config,log_dir,run_id,zk_connector,fe_address).run()
